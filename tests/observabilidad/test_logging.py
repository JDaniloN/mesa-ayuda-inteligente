...
def test_reintento_ia_exitoso_registra_fallo_y_resultado_correlacionados(capsys):
    intentos = {"total": 0}

    def handler(_request: httpx.Request) -> httpx.Response:
        intentos["total"] += 1
        if intentos["total"] == 1:
            return httpx.Response(500, json={"error": "transitorio"})
    ...
    respuesta = cliente.post(
        ...
        headers={"Authorization": f"Bearer {token}"},
        json={
            "asunto": "No abre la aplicación contable",
            "descripcion": "Bloquea el cierre mensual",
            "area": "Aplicaciones",
            "solicitante": "persona@empresa.test",
        },
    )

    eventos = _eventos(capsys.readouterr().out)
    request_id = respuesta.headers["X-Request-ID"]
    por_nombre = {evento["event"]: evento for evento in eventos}
    assert respuesta.status_code == 201
    assert intentos["total"] == 2
    assert por_nombre["ia_intento_fallido"]["reason"] == "http_500"
    assert por_nombre["ia_intento_fallido"]["attempt"] == 1
    assert por_nombre["ia_clasificacion_completada"]["attempt"] == 2
    assert por_nombre["ia_clasificacion_completada"]["classification_origin"] == "proveedor"
    assert all(evento["request_id"] == request_id for evento in eventos)


def test_excepcion_no_controlada_registra_tipo_sin_detalle_y_con_request_id(capsys):
    detalle = "dato interno reservado"
    ...
    evento = next(
        evento
        for evento in _eventos(salida)
        if evento["event"] == "http_request_failed"
    )
    assert respuesta.status_code == 500
    assert evento["request_id"] == respuesta.headers["X-Request-ID"]
    assert evento["exception_type"] == "RuntimeError"
    assert detalle not in salida
